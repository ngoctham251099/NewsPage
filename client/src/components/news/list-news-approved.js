...
function ListNews(props) {
  ...
  let d = 0;
  useEffect(() => {
    axios.get("http://3.130.135.8/api/api-news/list-news-approved").then((res) => {
  ...
  const updateStatus = async (idNews) => {
    const id = localStorage.getItem("idUser");
    console.log(id);
    const res = await axios.post(`http://3.130.135.8/api/api-news/update-secretary/${id}/${idNews}`, {
  ...
  const ViewsId = (id) => {
    axios.post(`http://3.130.135.8/api/api-news/view/${id}`).then((res) => {
      if (res.data) {
        setView(true);
      } else {
      }
